...
<body>
	...
	">
		...
		<nav>
			...
			<ul class="topo-items" >
				<li><img src="<?php echo esc_url( get_template_directory_uri() ); ?>/logo.png" alt="Quatro Rodas" title="Quatro Rodas" />
				</li>
	...
	<script>
		...
		$(document).ready(function() {
			$(".burger-nav").on("click",function() {
				$("ul.topo-items").toggleClass("open");
			});
			
			$("ul.topo-items > li").on("click",function() {
		...
		$(window).on('scroll', function () {
			...
			if (scrollTop > 20 && $(window).width() > 1266) {
				$('nav.acessados').stop().animate({"margin-top": "27px"},20);
				$('body header').stop().animate({"height": "187px"},20);
			...
			if (scrollTop < 20 && $(window).width() > 1266) {
				...
				$('body header').stop().animate({"height": "207px"},20);
